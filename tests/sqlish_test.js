...
Y.Test.Runner.add(new Y.Test.Case({
        name: "Basic tests through version 0.0.11",
        "Should do basic SQL assembly": function () {
        ...
        assert.areEqual(s, expected_s, "#5\n" + s + "\n" + expected_s);
        ...
        assert.areSame(threw_error, true, "PostgreSQL 9.2 should throw an error on REPLACE");
        ...
        assert.areSame(threw_error, true, "Dialect SQLite3 should throw error on set()");
    },
    
    
    // Setup some basic tests for SQLite support
    "Should pass SQLite specific tests." : function () {
        ...
        Y.assert(wasThrown, "Should have thrown an error for Sql.set()");
        ...
        Y.assert(wasThrown, "Should have thrown an error for Sql.into(\"@myId\")");
    },
    
    
    //Testing toString() terminations
    "Should handle toString() terminations": function () {
        var sql = new sqlish.Sqlish();
        
        assert.areEqual(sql.eol, ";", "Should have eol equal to ;");
        assert.areEqual(sql.select("count()").toString(), "SELECT count();");
        assert.areEqual(sql.select("count()").toString(""), "SELECT count()");
    },
    
    
    // Test 0.0.3 feature set
    "Should have version 0.0.3 features.": function () {
        var sql = new sqlish.Sqlish(),
            s,
            expected_s;
    ...
    "Should have version 0.0.4 feature set": function () {
        var sql = new sqlish.Sqlish(),
            s,
            expected_s;
    ...
    "Should avoid injection vulnerabilities": function () {
        var sql = new sqlish.Sqlish(),
            threw_error = false,
            s,
            expected_s,
        ...
        Y.assert(threw_error, "injection error expected");
        ...
        assert.areSame(threw_error, true, "Should throw an error when injection attempted on tableName parameter.");
        ...
        assert.areSame(threw_error, true,
        ...
        assert.areNotEqual(sql.schemas, undefined, "Should have schemes defined");
        sql.on("test", {
        ...
        assert.areNotEqual(sql.schemas.test, undefined, "Should have sql.schemas.test defined");
        sql.on("profiles", {
            profile_id: function (value) {
                if (value === "profile_id" ||
        ...
        assert.areNotEqual(sql.schemas.profiles, undefined, "Should have sql.schemas.profiles defined.");
    
        assert.areEqual(sql.safely("test.profile_id", sql.schemas),
                     "test.profile_id",
                     "Should find column: " + sql.isColumnName("test.profile_id", sql.schemas));
        assert.areEqual(sql.safely("profiles.profile_id", sql.schemas),
                     "profiles.profile_id",
    ...
    "Should have 0.0.5 fearture set": function () {
        var sql  = new sqlish.Sqlish(),
            s,
            expected_s,
            threw_error = false,
        ...
        assert.areSame(threw_error, true, "$like should not accept an object as an argument.");
        ...
        assert.areSame(threw_error, true, "dialect of PostgreSQL should throw error when replace() is called.");
        ...
        assert.areSame(threw_error, true, "Calling createView() with string in second parameter should throw error");
    ...
    "Should support fix 0.0.6 bugs.": function () {
        ...
        Y.assert(sql.select("id").from("event").toString(), "select().from()");
        // BUG: by the time where() is called dialect was getting lost.
        Y.assert(sql.select("id").from("event").where({$and: [{title: row.title}, 
    ...
    "Should fix 0.0.8 bugs": function (test_label) {
        var sql = new sqlish.Sqlish("MySQL 5.5"),
            threw_error = false;
        ...
        try {
            ...
            Y.assert(err, "Should have throw injection error ->" + err);
            threw_error = true;
        }
        assert.areSame(threw_error, true, "Should have thrown an error on " + expected_s);
        assert.areNotEqual(s, expected_s, "\n" + s + "\n" + expected_s);
    ...
    "Should support a more complex join": function (test_label) {
        var blog_id = 59,
            post_id = 43587,
            sql = new sqlish.Sqlish("MySQL 5.5"),
            s,
    ...
    "Should support SQLite3 type escaping": function (test_label) {
        var sql = sqlish.Sqlish("SQLite3");
    
        s = sql.safely("O'Malley's");
        assert.areEqual(s, "\"O''Malley''s\"", "Should have SQLite3 escaping, s was " + s);
    }
}));
